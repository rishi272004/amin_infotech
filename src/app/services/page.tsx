        ...
        <section className="bg-slate-50 py-24 mb-16 relative overflow-hidden">
          ...
          <div className="container-custom relative z-10">
            <div className="max-w-4xl">
              <span className="text-rose-600 font-black tracking-[0.3em] text-[11px] uppercase mb-6 block">Our Capabilities</span>
        ...
        <div className="container-custom pb-32">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {services.map((service) => (
              <div key={service.id} className="group bg-slate-50 rounded-[2.5rem] border border-slate-100 p-8 hover:bg-white hover:border-rose-100 hover:shadow-2xl transition-all duration-500">
                <div className="bg-rose-600 h-1.5 w-12 rounded-full mb-8"></div>
                <h2 className="text-2xl font-black text-slate-950 mb-4 tracking-tight group-hover:text-rose-600 transition-colors">{service.name}</h2>
                <p className="text-slate-500 font-medium text-sm leading-relaxed mb-8">{service.description}</p>
                <a
                  href={service.link}
                  className="inline-flex items-center gap-2 text-rose-600 font-bold text-sm group/link"
                >
                  Learn More <span className="translate-x-0 group-hover/link:translate-x-1 transition-transform">→</span>
                </a>
              </div>
            ))}
          </div>
